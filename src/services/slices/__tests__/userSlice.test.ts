...
describe('userSlice reducer', () => {
  ...
  const testUser = { name: 'Umar', email: 'khamadov@mail.ru', password: 'password' };
  ...
  describe('Асинхронные действия', () => {
    ...
    test('обработка loginUser.fulfilled', () => {
      const actual = userReducer(initialState, loginUser.fulfilled(testUser, '', { email: 'khamadov@mail.ru', password: 'password' }));
      expect(actual.data).toEqual(testUser);
      expect(actual.requestStatus).toBe(RequestStatus.Success);
    });
